...
export const AutocompleteDropdown = memo<
  React.ForwardRefExoticComponent<
    Omit<IAutocompleteDropdownProps, 'ref'> & React.RefAttributes<unknown>
  >
>(
  forwardRef((props: IAutocompleteDropdownProps, ref) => {
    ...
    const InputComponent =
      (props.InputComponent as typeof TextInput) || TextInput;
    ...
    const initialDataSetRef = useRef<AutocompleteDropdownItem[] | null>(
      dataSetProp,
    );
    ...
    const calculateDirection = useCallback(
      async ({waitForKeyboard}: {waitForKeyboard: boolean}) => {
        const [, positionY] = await new Promise<
        ...
        return new Promise<void>(resolve => {
          setTimeout(
            () => {
              ...
              setDirection(
                (screenHeight - kbHeight) / 2 > positionY ? 'down' : 'up',
              );
              resolve();
            },
            waitForKeyboard
              ? Platform.select({ios: 600, android: 250, default: 1})
    ...
    useEffect(() => {
      ...
      if (typeof initialValue === 'string') {
        ...
        initialValueItem = initialDataSet?.find(
          el => el.id === initialValue?.id,
        );
        if (!initialValueItem) {
    ...
    useEffect(() => {
      const methods = activeControllerRef
        ? {close, blur, open, toggle, clear, setInputText, setItem}
        : null;
    ...
    useEffect(() => {
      ...
      const newSet = initialDataSet.filter((item: AutocompleteDropdownItem) => {
        ...
        if (matchFromStart) {
          return (
            typeof item.title === 'string' && findWhere.startsWith(findWhat)
          );
        } else {
          return (
            typeof item.title === 'string' && findWhere.indexOf(findWhat) !== -1
          );
        }
      });
    ...
    const ListEmptyComponent = useMemo(() => {
      return (
        EmptyResultComponent ?? (
          <NothingFound emptyResultText={emptyResultText} />
        )
      );
