...
export const AutocompleteDropdownContextProvider: FC<
  IAutocompleteDropdownContextProviderProps
> = ({headerOffset = 0, children}) => {
  ...
  const [inputMeasurements, setInputMeasurements] = useState<
    | {x: number; topY: number; bottomY: number; width: number; height: number}
    | undefined
  >();
  ...
  useEffect(() => {
    ...
    if (dropdownHeight && direction === 'up') {
      ...
      setContentStyles({
        top:
          inputMeasurements.topY + inputMeasurements.height + 5 + headerOffset,
        bottom: undefined,
        left: inputMeasurements.x,
        width: inputMeasurements.width,
      });
  ...
  const recalculatePosition = useCallback((showAfterCalculation = false) => {
    activeInputContainerRef?.current?.measure(
      (x, y, width, height, inputPageX, inputPageY) => {
        wrapperRef.current?.measure(
          (
            ...
            setInputMeasurements(prev =>
              JSON.stringify(prev) === JSON.stringify(currentMeasurement)
                ? prev
                : currentMeasurement,
            );
